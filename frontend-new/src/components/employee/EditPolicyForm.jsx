            ...
            <div className="max-w-5xl mx-auto px-6 py-12">
                ...
                <div className="flex justify-between items-center mb-10 pb-6 border-b border-slate-200">
                    <div className="flex items-center">
                        ...
                        <div>
                            <h2 className="text-3xl font-bold text-slate-900 tracking-tight">Edit Policy Entry</h2>
                            <p className="text-slate-500 mt-1 text-lg uppercase tracking-[2px]">Update quotation, issuance, and ledger details.</p>
                        </div>
                    </div>
                    {onCancel && (
                ...
                <form onSubmit={handleUpdate} className="space-y-8 pb-20">
                    ...
                    <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-xl shadow-slate-200/50">
                        <div className="mb-8 flex items-center justify-between border-b border-slate-100 pb-5">
                            ...
                            <div className="px-3 py-1 bg-slate-50 rounded-lg border border-slate-200 text-[10px] font-bold text-slate-400 uppercase tracking-widest ">
                                REF: {policy?.id?.slice(-8).toUpperCase() || policy?._id?.slice(-8).toUpperCase()}
                            </div>
                        </div>
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                        <div className="bg-white rounded-[40px] border border-slate-200 overflow-hidden shadow-2xl">
                            <div className="px-8 py-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                                ...
                                <button type="button" onClick={() => setPaymentsOut([...paymentsOut, { category: 'Payment to Insurance Company', companyName: '', amount: '', date: new Date().toISOString().split('T')[0], paymentLinkType: 'GI', paymentMode: 'Bank', accountNumber: '' }])} className="text-[10px] font-bold uppercase tracking-widest flex items-center bg-rose-600 text-white px-5 py-3 rounded-2xl shadow-lg shadow-rose-600/20 hover:bg-rose-700 transition-all active:scale-95">
                                    <Plus size={16} className="mr-2" /> Add Entry
                                </button>
                            </div>
                            <div className="p-8 space-y-6">
                                {paymentsOut.map((p, idx) => (
                                    <div key={idx} className="p-6 rounded-[28px] border border-slate-100 bg-slate-50/30 relative group shadow-sm transition-all hover:border-rose-100">
                                        <button type="button" onClick={() => removePaymentOut(idx)} className="absolute top-4 right-4 text-slate-300 hover:text-rose-500 transition-colors opacity-0 group-hover:opacity-100"><Trash2 size={18} /></button>
                                        <select value={p.category} onChange={e => updatePaymentOut(idx, 'category', e.target.value)} className="w-full mb-5 bg-white text-rose-500 p-3 rounded-xl text-[10px] font-bold border border-slate-200 uppercase outline-none focus:ring-2 focus:ring-rose-500/10">
                                            <option>Insurance Premium</option>
                                            <option>Link Payment</option>
                                            <option>Commission</option>
                                        </select>
                                        <div className="grid grid-cols-2 gap-4">
                                            <input placeholder="Provider" value={p.companyName} onChange={e => updatePaymentOut(idx, 'companyName', e.target.value)} className="p-3 bg-white border border-slate-200 text-slate-900 text-xs rounded-xl font-bold placeholder:text-slate-300" />
                                            <input type="number" placeholder="Amt" value={p.amount} onChange={e => updatePaymentOut(idx, 'amount', e.target.value)} className="p-3 bg-white border border-slate-200 text-slate-900 text-xs rounded-xl font-bold placeholder:text-slate-300" />
                                            <select value={p.paymentLinkType} onChange={e => updatePaymentOut(idx, 'paymentLinkType', e.target.value)} className="p-3 bg-white border border-slate-200 text-slate-900 text-xs rounded-xl font-bold"><option>GI</option><option>Link</option></select>
                                            <select value={p.paymentMode} onChange={e => updatePaymentOut(idx, 'paymentMode', e.target.value)} className="p-3 bg-white border border-slate-200 text-slate-900 text-xs rounded-xl font-bold"><option>Bank</option><option>CC</option><option>Other</option></select>
                                            <input type="date" value={p.date} onChange={e => updatePaymentOut(idx, 'date', e.target.value)} className="p-3 bg-white border border-slate-200 text-slate-900 text-xs rounded-xl font-bold" />
                                            <input placeholder="Ref No" value={p.accountNumber} onChange={e => updatePaymentOut(idx, 'accountNumber', e.target.value)} className="p-3 bg-white border border-slate-200 text-slate-900 text-xs rounded-xl font-bold placeholder:text-slate-300" />
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ...
                        <div className="bg-white rounded-[40px] border border-slate-200 overflow-hidden shadow-2xl">
                            <div className="px-8 py-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                                ...
                                <button type="button" onClick={addReceiptIn} className="text-[10px] font-bold uppercase tracking-widest flex items-center bg-emerald-600 text-white px-5 py-3 rounded-2xl shadow-lg shadow-emerald-600/20 hover:bg-emerald-700 transition-all active:scale-95">
                                    <Plus size={16} className="mr-2" /> Add Entry
                                </button>
                            </div>
                            <div className="p-8 space-y-6">
                                {receiptsIn.map((r, idx) => (
                                    <div key={idx} className="p-6 rounded-[28px] border border-slate-100 bg-slate-50/30 relative group shadow-sm transition-all hover:border-emerald-100">
                                        <button type="button" onClick={() => removeReceiptIn(idx)} className="absolute top-4 right-4 text-slate-300 hover:text-rose-500 transition-colors opacity-0 group-hover:opacity-100"><Trash2 size={18} /></button>
                                        <div className="grid grid-cols-2 gap-4 mb-5">
                                            <select value={r.fromType} onChange={e => updateReceiptIn(idx, 'fromType', e.target.value)} className="bg-white text-emerald-600 p-3 rounded-xl text-[10px] font-bold border border-slate-200 uppercase outline-none">
                                                <option>Owner</option>
                                                <option>Agent</option>
                                            </select>
                                            <select value={r.paymentMode} onChange={e => updateReceiptIn(idx, 'paymentMode', e.target.value)} className="bg-white text-indigo-600 p-3 rounded-xl text-[10px] font-bold border border-slate-200 uppercase outline-none">
                                                <option>Cash</option>
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            <div className="col-span-2 relative">
                                                <input type="number" placeholder="Amt Received" value={r.amount} onChange={e => updateReceiptIn(idx, 'amount', e.target.value)} className="w-full p-4 pl-12 bg-white border border-slate-200 text-emerald-600 text-sm font-bold rounded-xl placeholder:text-slate-300 outline-none focus:ring-2 focus:ring-emerald-500/10" />
                                                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-emerald-600 font-bold text-lg">₹</span>
                                            </div>
                                            <input type="date" value={r.date} onChange={e => updateReceiptIn(idx, 'date', e.target.value)} className="p-3 bg-white border border-slate-200 text-slate-900 text-xs rounded-xl font-bold" />
                                            <input placeholder="Ref/Acc" value={r.paymentMode === 'Credit' ? r.creditDetails : r.bankAccountNumber} onChange={e => updateReceiptIn(idx, r.paymentMode === 'Credit' ? 'creditDetails' : 'bankAccountNumber', e.target.value)} className="p-3 bg-white border border-slate-200 text-slate-900 text-xs rounded-xl font-bold placeholder:text-slate-300" />
                                        </div>
                                    </div>
                                ))}
                            </div>
